...
and pooled into the compact readout. The `kiro-cli /usage` scrape (a local slash
command over the same free API) remains fallback-only, and its two known bonus
formats produce the same list shape.
...
- Timer loop: non-blocking execution via independent `asyncio.create_task()` per job (via `_run_job_isolated()`), with a strong ref on the job's run claim (`_RunClaim.task`) to prevent GC. One hung ACP session no longer freezes the entire cron system. Per-job timeout increased from 5 min to 30 min (`_JOB_TIMEOUT_SECS`); zombie detection (`is_responsive()` after 10 min inactivity) provides additional defense-in-depth
- Reaper clock: the `_reaper_loop` backstop decides on the **monotonic** clock, not the wall clock — it compares `time.monotonic() - claim.started_monotonic` against the deadline, where the claim's monotonic start is stamped in `_run_job_isolated` and goes with the claim's one release (its `finally`, the reaper sweep's finished-task release `discard_finished_run`, `_force_reap`, `cancel`). The primary guard it backs up is `_execute_with_timeout`'s `asyncio.wait_for`, whose deadline also runs on the loop's monotonic clock, so measuring on a different clock makes the two disagree whenever the wall clock jumps: a job spanning a **host suspend** looks like it ran for the whole sleep window and is force-killed with a spurious `status=timeout` in history, while `wait_for` (correctly) never fired. A claim with no monotonic stamp (a manual run still parked in its store refresh) falls back to the wall-clock elapsed from the claim's `claimed_at`, so the backstop never stops reaping. `claimed_at` is the wall-clock stamp and remains what human-facing surfaces read — `running_since()`, history timestamps, and the reaper's own "ran Ns" log line. Consequence to expect: a suspend-spanning job is no longer killed on wake and survives until the monotonic `wait_for` deadline, then records a genuine timeout. **The sweep kills on the stored claim, never on its snapshot**: `_reaper_loop` iterates `list(self._claims.items())` for order only and awaits inside `_force_reap` for every overdue job it meets (the session reset, the locked persist, the history append), so by the time it reaches a later job that job's run may have ended and a replacement claimed it — the snapshot's claim then belongs to a run that is gone, with stamps that put the job far past its deadline. Each iteration therefore first re-reads the store and skips the entry unless the stored claim is the very object it holds (identity), and hands that claim to `_force_reap`, which takes it only while it is still the stored one (`_take_claim(job_id, expected=claim)`) and otherwise kills nothing — a replacement is measured on its own stamps by the next sweep, exactly as the retired per-job dicts, read live per iteration, let a replacement's fresh stamp make the sweep `continue`. `_force_reap` has no job-id-only form: `claim` is required and the sweep is its only caller, so a kill always answers for one named run — there is no path that kills a job's session with no run to record it against. The claim it took, the reap finishes in a `finally` around its kill (the lock-contract bullet above): the sweep swallows and logs a reap's exception, so a claim an aborted reap left taken would otherwise be skipped by every later sweep.
...
On-demand job execution via CLI (`kirocrew cron trigger <job_id>`) and MCP tool (`cron_trigger`). Delegates to `POST /api/crons/{id}/run`. The endpoint returns job name and uses `create_task` for non-blocking execution. If a run for the job is already in flight (the job holds a run claim: `is_running(job_id)`), the endpoint returns **409** `{"error": "job is already running"}` instead of starting a second overlapping run — the guard is an atomic check-and-claim with no `await` between the check, `run_job`'s claim and `attach_run_task`, so overwriting (and thereby orphaning) the in-flight task handle is impossible. A tracked task that has already finished does **not** count as in flight: before the check the handler calls `CronService.discard_finished_run(job_id)`, which drops the run's leftover claim (task handle, start stamps, jitter and all) when — and only when — the claim's task is `done()` and no `cancel()` or reap has taken the claim (a taken claim is its teardown's to pop once its kill awaits return), so a run whose task ends without reaching `_run_job_isolated`'s `finally` cannot make the endpoint refuse the job; a task still running keeps the 409. The reaper sweep (`_reaper_loop`, every `_REAPER_INTERVAL`) applies the same release to every finished task it meets whose claim is still stored — ahead of its deadline math, not once the run's deadline passes — so the scheduled due-scan, `_next_wake_secs` and `run_job`, which all skip a claimed job, see the job idle again within one sweep instead of passing over every scheduled fire until a manual run or a gateway restart. `CronService.cancel()` is the third site: it calls the same release before its claim guard, so `POST /api/crons/{id}/cancel` against a finished task answers **409** `{"error": "job is not running"}` honestly with the leftovers released, instead of killing nothing, recording a `cancelled` history row for a run that ended long ago, and leaving `_cancelled_jobs` set for a `finally` that never runs — which made the job's next real run skip its merge and history as if cancelled. Unknown job → 404. This 409 propagates to the dashboard "Run now" button, the CLI `trigger`, and the `cron_trigger` MCP tool, which surface it as an "already running" outcome rather than a duplicate launch. Both CLI and MCP paths include SEL audit logging.
...
- `handlers/teams.py` — crewmate teams, the Crewmates page's roster grouping and team view. A team is a name plus an ORDERED list of exact crew names (`kiro_crew.crew_teams.Team` — `crew_teams`, because `kiro_crew.teams` is the Microsoft Teams channel package and a same-named module would be shadowed by it), nothing else: no memory, no chat, no prompt of its own; everything the team view shows is derived from its members' existing threads, activity records and live presence. Storage is one document, `teams.json` in its own data-home directory `crew-teams/` (`{"version": 1, "teams": [{id, name, members}]}` — `TEAMS_SCHEMA_VERSION`, written on every save; a document from a NEWER build is refused whole). The directory is deliberately NOT under `trust/`: `trust` is a declared sandbox READ-WRITE exception (in-sandbox code appends to the audit log there), so a record under it stays writable by a sandboxed command that builds the path at runtime — the `crew-panels` lesson. `crew-teams` is instead a whole-directory leaf on `sandbox._CREW_HIDDEN_LEAVES` (masked from every sandboxed process) and `sandbox._CREW_PRECREATE_HIDDEN_DIR_LEAVES` (materialised empty at 0700 before every spawn, because the store is created on the first team and `mount(2)` cannot mask an absent name) and on `security._CREW_SECRET_LEAVES` (agent file tools refuse read and write); only the gateway, on a human dashboard action, opens it, so a crewmate's own tools cannot re-team itself or its siblings (`test_store_directory_is_masked_and_fenced`). Every mutation runs read → mutate → rewrite under TWO locks — a process-wide `threading.Lock` (`_WRITE_LOCK`) and, inside it, an advisory file lock on `crew-teams/.lock` (`platform_compat.file_lock`, `crew_teams.document_lock`, public only for the cross-process test; NOT re-entrant — no caller holds it across another store call): the gateway's writers (owner routes, the crew create/delete/sync hooks) run off-loop in worker threads, `kirocrew agent create` / `delete` write from ANOTHER process, and only the file lock reaches it; a contended acquire past the default ceiling fails closed as `OSError` → 500 `teams_write_failed` rather than writing unserialized (`test_writers_hold_a_cross_process_file_lock`) — atomically with fsync (`crew_teams.write_teams`; the parent-directory sync after the committed rename is best-effort, because the rename has already published the document and a 500 there would make a retry create the team twice — `test_a_failed_directory_sync_does_not_fail_a_committed_write`), and the read (`crew_teams.read_teams`) treats an ABSENT file as no teams but an existing-yet-unparseable one — or one past any cap it declares: more than `TEAMS_MAX` teams, a non-12-hex id, a blank or over-`TEAM_NAME_MAX_CHARS` name, more than `TEAM_MEMBERS_MAX` members, an empty or over-`MEMBER_NAME_MAX_CHARS` member, or a name or member holding an escaped lone surrogate (legal JSON, unencodable as UTF-8 — retained, it would make the next mutation's own encode raise mid-write) (`test_read_refuses_a_document_past_the_caps`) — as `TeamsUnreadable`, which every route answers as 500 `teams_unreadable` rather than an empty list — reading it empty would let the next write erase every team the user made. There is deliberately NO repair route: a route that reset the document would have to read the document it is repairing, and an unreadable document also refuses crew creation (below), so the accepted remedy is the operator's shell — fix the `teams.json` document inside the data home's `crew-teams` directory, or remove it, since an absent file reads as no teams. The store logs exactly that, with the path, every time a read fails (`crew_teams._log_unreadable`); the API answers never carry the path, because a dashboard reader is not necessarily the host's operator, and `kirocrew agent create` prints it on stderr. Two invariants the store enforces so no UI has to: a crewmate is on AT MOST ONE team (`create_team` / `update_team` detach the listed names from every other team in the same write; a foreign document listing a name twice reads as the FIRST team, `_parse_teams`), and members are bounded on WRITE exactly as on read — a member past `MEMBER_NAME_MAX_CHARS` or one UTF-8 cannot encode is refused as `invalid_members` BEFORE the registry lookup (`test_a_member_the_reader_would_refuse_is_never_written`), and the handler's registry view uses `_AGENT_NAME_RE.fullmatch` so a trailing newline cannot make a 65-character key "known" — and names are validated against the registered crews at write time (`unknown_member`), never against slugs — slugification is lossy and the roster is keyed by exact name. Ids are 12 random hex chars (`_new_team_id`), path- and query-safe; the routes refuse any other spelling with 400 `invalid_team_id` before touching the file. Names are stripped, capped at `TEAM_NAME_MAX_CHARS` (refused, never truncated), and refuse lone surrogates plus Unicode category `Cc` / `Zl` / `Zp` — control characters and line separators (a roster header is not a place for a terminal-control payload); format characters (a ZWJ emoji sequence) and the no-break space are legitimate in a pasted name and pass. `drop_member` is the crew-DELETE path's hook (`api_kirocrew_agent_delete`, best-effort off-loop): a team must not keep counting a crewmate with no roster row; a stale name is otherwise tolerated by every reader, which is why BOTH an unreadable document and a failed rewrite (`OSError`) are logged and swallowed there — the crew's config write has already committed, and neither may turn a completed delete into a 500. The document is bounded on WRITE as well as read, and the read itself is bounded: `read_teams` reads at most `TEAMS_FILE_MAX_BYTES + 1` bytes (`atomic_write.read_bytes_with_retry(max_bytes=…)`) and refuses a longer document, so a restored file far past the cap is never allocated whole on the way to its refusal (`test_an_oversized_document_is_refused_without_being_read_whole`). `TEAMS_FILE_MAX_BYTES` is sized above `max_document_bytes()` (the largest document every cap admits, pinned by `test_a_valid_write_can_never_outgrow_the_read_cap`), and a save that would exceed it is refused with 400 `teams_too_large` before anything lands, so a valid sequence of writes can never leave the store `teams_unreadable`. Two removal paths never reach that route's hook — `kirocrew agent delete` runs in another process, and the package sync prunes agents whose package is gone — so both call `drop_member` with the SAME best-effort contract (the CLI after its registry write, never refusing a delete over the grouping store — `test_delete_drops_the_crew_from_its_team_best_effort`; the sync prune ONLY for the names its locked mutate actually deleted, a snapshot candidate edited concurrently survives and keeps its team) — AND `GET /api/teams` reconciles the document against the live registry on every read (`crew_teams.prune_unknown`): an API answer never names a crewmate the registry lacks, whatever path removed it, while the document itself is left as written and re-validated by the next membership write. The one harm a stale entry could still do — a crew deleted and later RECREATED under the same name silently rejoining its old team, because `prune_unknown` hides a gone name but not a re-known one — is closed at the only moment it can materialize, the CREATE: every create path (`api_kirocrew_agents_create`, `kirocrew agent create`, the package sync for each name it is about to add) purges it from any team it was left on with `crew_teams.release_for_create` — NOT as a caller's option but inside `persist_member_config` itself for every `create=True` publication (the dashboard, the CLI, an app's `ensure_team`; the package sync, which registers through its own locked mutation, calls it from there), INSIDE the registry's cross-process locked mutation, after that mutation has found the name free and immediately before it registers it. Inside the lock, not before it: a purge outside the lock races a same-name create-and-team in another process and would erase THAT crew's fresh membership (`test_every_create_purges_the_name_inside_the_locked_mutation`). Symmetrically every removal path runs `drop_member` through `update_config_locked(after_write=…)` — AFTER the registry write has committed and still INSIDE its lock (the dashboard delete, the CLI delete, the sync prune): after the commit, so a registry write that fails leaves the membership as it was (the crew stays, on its team), inside the lock, so a same-name create cannot land between the delete and the drop (`test_after_write_runs_only_once_the_registry_write_committed`). Unlike `drop_member` this is NOT best-effort — registering the name while its stale membership cannot be purged is exactly what would expose it — so `TeamsUnreadable` / `OSError` refuse the create: 409 `teams_unavailable` on the route, exit 1 `cannot create agent` from the CLI, and the sync simply leaves that name out of THIS run (`deferred_names`) — out of its `synced` answer and its audit record too, a deferral is not a registration — and picks it up next time (`test_release_name_purges_a_recreated_crews_stale_membership`, `test_release_name_propagates_a_failure_so_the_create_is_refused`, `test_create_releases_the_name_from_a_stale_team_first`, `test_create_is_refused_while_a_stale_membership_cannot_be_purged`). An ABSENT document — no team was ever made — answers False and writes nothing (`test_release_name_without_a_store_writes_no_document`), but absence is decided INSIDE the document lock, not before it: a restore or import installing a document in the gap between a pre-lock stat and the registration would let the name inherit that document's membership, and every such installer holds this same lock (`test_release_name_decides_absence_under_the_lock`); absence is decided by `stat` raising `FileNotFoundError`, never by `Path.exists`, which folds every `OSError` into False and would let a directory the process cannot stat into read as "no teams" (`test_release_name_does_not_read_an_unstatable_store_as_absent`). Routes: `GET /api/teams` (`{teams}`), `POST /api/teams {name, members}` → 201 `{team}`, `PUT /api/teams/{id} {name?, members?, add?, remove?}` → `{team}` (omitted fields unchanged; all omitted is 400 `nothing_to_update`; `members` REPLACES the list while `add` / `remove` are deltas applied to the list as it stands inside the document lock, so an editor holding a stale copy changes only the crewmates it touched — `_validate_names` bounds a removal without a registry check, since a deleted crew's stale entry is what a caller may want gone; the two shapes are exclusive, `invalid_members`; `test_membership_deltas_never_write_a_stale_snapshot_back`), `DELETE /api/teams/{id}` → `{ok}`; unknown id is 404 `team_not_found`, malformed bodies are coded 400s (`invalid_json`, `invalid_team_name`, `team_name_too_long`, `invalid_members`, `unknown_member`, `too_many_teams`, `too_many_members`, `teams_too_large`), a failed persist is 500 `teams_write_failed`. The create and update routes run INSIDE the crew registry's in-process config lock (`agents._get_config_lock`, resolved late), the same lock the dashboard's crew-delete route holds from removing the crew through `drop_member` and the crew-create route from `release_name` through registering the name; and the registry read itself happens INSIDE the store's document lock — `known` is a callable (`crew_teams.KnownCrews`, `handlers/teams._registered_crews`) the store evaluates after acquiring the lock, so the snapshot a write validates against is the freshest available (`test_registry_is_read_inside_the_document_lock`). Lock order everywhere is config lock (the in-process asyncio lock and the cross-process sidecar lock), then document lock; `KiroCrewConfig.load` takes no lock, so the `known` read inside the document lock cannot invert it. So the in-process delete cannot interleave between "the crew exists" and "the team is written"; `kirocrew agent delete` in another process still can, and the team then names a gone crewmate — hidden by `prune_unknown`, purged by `release_name` if the name ever comes back (`test_writers_hold_a_cross_process_file_lock` — a real second process holds the file lock, on every platform). Same posture as the members routes: app tokens are denied with 404 on every route (`_deny_app_caller`), and the three writes are OWNER actions (`_require_owner_write` → `require_owner_dashboard_request`, gated BEFORE body validation so a non-owner's answer stays a uniform 401/403 that leaks nothing about what would have validated; the shared gate audits only the denial, so the handler records the owner's ALLOW as a SEL `log_api_access(outcome="allowed")` event, enqueued and non-fatal — `test_an_owner_write_records_the_allow_in_sel`). Ids are matched with `fullmatch` on read and in the route, since `$` alone admits a trailing newline. The document rides every backup path: `crew-teams` is a snapshot component of its own (`snapshot.COMPONENTS`, a whole tree, so replace swaps it and merge copies it in where absent; the tree's `.lock` is host-local and never rides, `snapshot._HOST_LOCAL_PATHS`), the dashboard export zip writes the `teams.json` document, under its `crew-teams` directory, beside the core files and the import installs it where the destination has none, and both paths — snapshot restore in either mode and the dashboard import in either mode — validate the document they would install with the store's OWN reader (`crew_teams.read_document`, the same bounded read and `_parse_teams` the live `read_teams` runs, via `snapshot._TREE_DOCUMENT_VALIDATORS`) before anything moves, so a restore refuses exactly what the live read would refuse — a directory or a link standing at the document's name is refused outright, in every mode — and never installs a file that fails every team route and every crew create. The restore WRITES the way every other writer does: `crew_teams.install_document` / `remove_document` take the store's lock (`document_lock(directory)`, the restored home's own `crew-teams/.lock`) and move ONLY the document — replace, merge and the failed-restore rollback never remove or swap the directory or its lock file (`snapshot._LOCKED_DOCUMENT_TREES`); a replace from a bundle that carries no document — a tree without it, or no `crew-teams/` tree at all, which is what a home that never made a team exports — REMOVES the live one, so the destination's own teams cannot outlive a reported full replace (`test_replace_from_a_bundle_with_no_team_tree_removes_the_live_document`), because a writer already inside the lock would otherwise hold an orphaned file and commit its stale document over the restored one; a removal syncs the directory after the unlink, under the same lock, so a power loss cannot resurrect the removed document (`remove_document`, as `install_document` and `write_teams` already do); the replace-mode rollback copy of the document is taken by the installer INSIDE that same lock hold (`save_existing`), not in the restore's earlier backup phase, and is staged beside its name and published by rename only once the copy completed (a copy that fails midway leaves nothing where recovery reads), so a team write committed before the install can never be lost from both the live store and the rollback; a failed restore's rollback puts that copy back VERBATIM under the lock (`crew_teams.restore_document`) — recovery restores the operator's state, it does not validate it — STREAMED into a temporary file beside the document and published by rename, never read whole (the saved copy is whatever size the live store held, which the reader's cap does not bound), and raises only `OSError`, which the rollback loop names per target instead of aborting on. Only a MISSING path counts as "no document" to these writers: a directory or link where the data home's `teams.json` belongs (inside `crew-teams/`) raises `OSError` from `install_document` / `remove_document` (`_has_document`), so a document-less replace fails and rolls back instead of reporting success with the entry still in place and every team route still answering `teams_unreadable`. `_parse_teams` reads a document nested past the interpreter's stack (`RecursionError`) as `TeamsUnreadable`, the same coded 500 as any other unparseable document (`TestBackupCoverage`).
...
**Theme / display**: GET `/api/theme/boot` (**unauthenticated** — same boundary as `/api/health`; returns `{mode, color, language, onboarded, import_onboarded, privacy_acked, crewmates_onboarded}` so the SPA can apply the workspace theme, pick its UI language, and resolve onboarding gates before the token flow completes; no secrets), GET/PUT `/api/config/theme` (read/persist workspace display prefs `{mode?, color?, language?, onboarded?, import_onboarded?, privacy_acked?, crewmates_onboarded?}`; `mode` restricted to `""`/`dark`/`light`/`system`; `language` is `""` (auto-detect from the browser) or a BCP-47 tag validated for **shape only** via `_LANGUAGE_TAG_RE`, so adding a shipped language stays a frontend-only data change). All four response sites are built by one helper (`handlers/core.py::_theme_payload`) so a newly-added display pref cannot be surfaced by some and omitted by others — see `config.md` → "Dashboard UI language".
...
**Sessions**: GET `/api/sessions` (paginated list; opt-in `exclude_open=1` drops every session a live slot already holds open — resolved through `slot_history_key`/`slot_transcript_key` so channel tabs whose transcript is their `linked_session_key` are recognised, and applied BEFORE `total`/`has_more` so one page's arithmetic matches what it returned. The sidebar's Older-sessions pane is the only caller: it renders the complement of the tab list above it, while the full inventory stays the default for memory consolidation and the command palette's recents. The same predicate protects a session from DELETE `/api/sessions`. A SECOND opt-in, `user_only=1`, drops rows whose key sits in a machine namespace — the literal list `subagent`, `secretary`, `wf`, `wf-pool`, `wf-unpooled`, `wf-worker`, `wf-author`, `wf-scope`, `channel` — a presentation judgement for this pane, not a shared roster, since the similar tuples in `handlers/_shared.py` (memory-mode dispatch) and `handlers/cron.py` (live-shared liveness) answer different questions and do not agree with each other. Literal rather than derived from `messaging.link._TELEMETRY_LOCAL_PREFIXES`, which exists to bound telemetry label cardinality and does not list `wf-unpooled`, `wf-worker` or `wf-scope`. Matched through `_in_namespace` so the persisted stem `subagent_<id>` and the live key `subagent:<id>` classify alike — that fold is the whole defect, since every other subagent guard is spelled with a colon and can never match a filename stem. Applied before `total`/`has_more` for the same pagination reason as `exclude_open`. `taskrunner` is deliberately absent: `POST /api/taskrunner/{id}/to-chat` opens a titled chat slot on `taskrunner:<task_id>:chat:<token>`, so the namespace holds conversations as well as runs, and listing a plain run is the declared residual — anything unlisted stays visible, which is the direction every residual here points. The Older-sessions pane is the only caller. `search_sessions()` and the Delete-all key set read the same directory and deliberately do NOT adopt THIS narrowing — note that Delete-all does apply `exclude_open`, as above, so the two opt-ins differ in reach: a content search and a bulk delete both still see machine transcripts the pane no longer lists. The pane can be empty while machine transcripts remain on disk; the bulk-delete control keeps its row-count gate, so it is not offered over an empty list, and `DELETE /api/sessions` still clears those transcripts when it runs), GET `/api/sessions/{key}` (detail), DELETE `/api/sessions/{key}` (permanent delete), GET `/api/sessions/usage` (kiro credit usage, cached 10 min; the background refresh tries the real CodeWhisperer RTS `GetUsageLimits` API first — the true used/limit/overage — posting to a hardcoded regional host keyed off the whoami profile ARN (`codewhisperer.us-east-1.amazonaws.com` by default, `q.eu-central-1.amazonaws.com` for `eu-central-1`; any other/missing region falls back to us-east-1); reading bearer tokens from `kiro-cli`'s own SQLite auth store under keys `kirocli:odic:token` (OIDC/Builder ID), `codewhisperer:odic:token` (legacy), `kirocli:social:token` (GitHub/Google social login), and `kirocli:external-idp:token` (Identity Center/org SSO), then falls back automatically to scraping `kiro-cli chat --no-interactive --agent kirocrew-lite /usage` stdout whenever the API path returns no plan — `/usage` is a kiro-cli slash command answered locally from the same free `GetUsageLimits` call, so the scrape costs no credits and nothing gates it except its own back-off: after `_USAGE_SCRAPE_FAILURE_THRESHOLD` (3) consecutive scrape attempts that fail (time out, raise) or yield no parseable plan it is parked for `_USAGE_SCRAPE_BACKOFF_SECS` (6 h), because each attempt is a minute-scale subprocess spent on output that cannot be parsed (kiro-cli format change, wedged CLI, revoked auth); while parked the cache keeps a same-identity prior reading as `stale` or serves `available: false` with the API's partial fields; the API path does NOT refresh the token it reads, because that store is kiro-cli's to renew — an OPERATIONAL dependency, not just an implementation note, since on an install where the sign-in is owned elsewhere and kiro-cli is never driven the stored token lapses with nothing renewing it, so `fetch_usage_limits` returns an `auth_state` naming an auth-class failure (`no_credential` when nothing unexpired was readable, `rejected` on a 401 from GetUsageLimits; a 403 is an entitlement problem and is deliberately NOT counted) and when the refresh then ends without a reading — the scrape failed too, or was parked — the unavailable payload carries `reason: "signin_required"` so the pill names the remedy; a refresh that ends without a reading for any other cause carries no reason. The scrape decision itself does not consult that classification, because an empty candidate list does not prove kiro-cli cannot authenticate: it may authenticate from a store this reader does not enumerate, so the scrape runs and a working scrape's number always wins), POST `/api/sessions/usage/refresh` (refresh the credit reading now — the account modal's **Refresh** button. Runs one refresh, the same API-first, scrape-second sequence the timer runs, and answers `{"usage": <cache payload>}` exactly like the GET, so the frontend parses both through one normalizer (`api/kiroUsage.ts`) and writes the result into the pill's `['kiro-usage']` query, so the top-bar pill updates with the modal. Any authenticated dashboard caller may use it: the read costs nothing. Guards in order: the same `reject_if_kiro_unverified` as the GET; single-flight — a POST while a refresh is in flight (the timer's or another click's) is refused with 409 `refresh_in_flight`, checked synchronously against the in-flight guard (which `_fetch_usage_bg` sets before its first await) so two requests on one loop cannot both pass, because a refresh is a whoami + scrape subprocess pair of up to `_USAGE_FETCH_DEADLINE_SECS` and two of them for one reading is waste; and the scrape back-off, REPORTED rather than pre-checked — the refresh runs the free API attempt regardless of the scrape's state, and only when the API yields no plan AND the scrape is parked — parked before this refresh, or parked BY this refresh's own failed, timed-out or errored attempt (the third miss) — does the response carry `skipped: "scrape_parked"` with `retry_after` (seconds until the park lifts), telling the UI no new reading was fetched and none will be until then: the `usage` it received is a same-identity prior reading dimmed `stale`, or an unavailable marker, exactly what `_cache_without_scrape` / `_cache_transient_failure` left in the cache. Identity invariant, for BOTH sources: a reading is published only when a whoami taken immediately BEFORE the credential read and one taken immediately AFTER it prove the same account (`_whoami_agreement` = `same`: at least one identity field — email, `start_url`, profile ARN — a non-empty string in both snapshots and equal, and no field either snapshot carries disagreeing). The API read is bracketed by the top-of-refresh whoami (only synchronous checks sit between the two) and a whoami adjacent to its publish — the API's own ARN matching the anchoring identity proves nothing about who is signed in by the time the numbers arrive — and on `same` its identity fields are coupled to the billed ARN against that adjacent whoami. The `/usage` scrape, which reads whichever account kiro-cli has signed in when it runs, is bracketed by a whoami taken right before the spawn (not the top one: the API attempt sits between) and one taken right after the child returns, before its output is parsed; on `same` the reading is labelled from the after-snapshot. A pair that DISAGREES (a field on one side only, or different values: a switch or a lapsed session) publishes nothing and keeps nothing on either branch (`_cache_transient_failure` judged against the after-snapshot). A pair with NO identity evidence on either side is `unproven`, not agreement — a switch between two empty snapshots is undetectable, and with no anchoring ARN the API accepted whichever stored token it found: the API reading is dropped and the refresh falls through to the scrape; a scrape reading in the same state has no source left to fall back to and reports unavailable, so a kiro-cli whose whoami prints no identity yields no reading at all, by design. The parked path carries the API's partial fields (plan name, reset date) onto its unavailable marker only when its own adjacent whoami proves the top-of-refresh account. Every failure path — the parked scrape, a scrape that yields no plan, a timeout — keeps a prior reading only when `_same_identity` proves it belongs to this refresh's whoami; an unproven or different identity reports `available: false` (plus `reason: signin_required` when the API's verdict was auth-class), so a switch to an account whose credential lapsed never shows the previous account's balance or email. The modal renders Refresh under the state's own notice for the two no-reading states — an `ErrorNotice` "Could not read your balance." for `failed` (a read that failed), a passive "No balance reading is available for this account yet." for `none` (no reading exists yet; the top-bar pill renders this state as a dimmed "open to refresh" dash ONLY while the selected harness is kiro-cli — `isKiroBackend` in `api/acpBackend.ts` reads `agent.acp_backend` positively against the kiro id, never as "not claude", and an unloaded config renders no dash — because on any other harness there is no kiro-cli balance to read and the pill stays hidden on that payload, as before) — and as a compact control beside every live reading — never for `api-key` (no readout exists for that auth type) or `signin-required` (the read needs the same lapsed sign-in) — with idle → pending (disabled, `aria-busy`, "Refreshing…") → success (meter from the refreshed payload plus "Checked at {{time}}"; a payload that comes back `stale` renders the dimmed meter WITHOUT the checked-at stamp and reports "The refresh did not return a new reading. Showing the earlier one." instead) → error (an `ErrorNotice` with the agent hand-off, which closes the modal, naming the paused scrape in minutes, the stale outcome, or — in the one sentence the standing failure notice uses, "Could not read your balance." — a plain failed refresh). A 409 (`refresh_in_flight`) is a refresh outcome like the others: it is surfaced through `ErrorNotice` with the agent hand-off ("A refresh is already running."), Refresh is held (disabled, not pending) while the notice shows, and the modal re-reads the `['kiro-usage']` query once ~3 s later so the other refresh's result shows without another press — the notice then clears by itself and Refresh is offered again. Refresh is likewise held while the parked notice is showing (its copy says when to try again). Notices never stack: in the `failed` state a refresh outcome notice REPLACES the standing "Could not read your balance." (one alert, one hand-off), and a `stale` reading's gray line — "Showing an earlier reading; the latest refresh did not return one." — yields to the stale-refresh notice while that is showing, so the fact is stated once. There is no config key, Settings control or cost copy anywhere on this surface), POST `/api/sessions/summarize` (one-line LLM summaries for a list of session keys — bounded to 8, generated on an ephemeral background session with the cheap Haiku model, best-effort; backs the `list_sessions` MCP tool's opt-in `summarize=true`), GET `/api/sessions/memory` (per-session and per-task memory footprint; returns `{sessions, tasks, totals, history}` — each session row carries: `key`, `title`, `slot_key`, `untitled`, `agent`, `channel` (the grouping dimension, resolved by `telemetry_channel_of(key)` from `kiro_crew.messaging.link` — bounded cardinality, same taxonomy as telemetry metrics), `pid`, `owns_runtime`, `rss_mb`, `procs`, `mcp`, `cpu_cores` (those four are totalled over ONE walk of the session's process subtree — `acp.runtime._iter_descendant_pids`, whose edges come from a host parent map built once per poll — and that walk is deliberately UNCAPPED, so each figure covers the whole tree: the shared walker's `platform_compat._SUBTREE_MAX_PROCS` frontier bound guards a looping `/proc` graph, which this walk instead handles by visiting each pid once, so adopting the number here would bound the reported figure rather than the work. Truncation is the worse failure of the two because a capped count reaches the card as a plain integer indistinguishable from a complete one and is therefore presented as exact, where `procs`/`mcp`/`cpu_cores` reserve `null` for UNMEASURABLE and never report 0 for a live tree; a bound on the walk's cost, if one is ever needed, is a work or time budget that yields `null` on exhaustion. `mcp` counts command lines through `platform_compat.process_matches`, the helper the platform-compat table's "Match process cmdline" row names), `prompts`, `uptime_s`, `credits` (cumulative kiro credits consumed over the spend window — `SPEND_WINDOW_DAYS` in `dashboard/handlers/usage.py`, which the Telemetry spend tab's `cost_breakdown` shares so the two surfaces cannot report different totals for one session; `null` when no measured turn exists in the window, which is semantically distinct from zero; the join is by session key, and because a slot bound to a channel or cron conversation runs its turns under `linked_session_key` while its usage rows are still filed under the dashboard `slot.key`, the lookup falls back to `DashboardState.spend_slot_by_session()` — without that reverse index those sessions report `null` despite having spent), `turns` (count of completed turns in the same window — `null` same semantics as `credits`), `parent` (the session that opened this one through `session_create`, as the child's own crew log records it — `null` for a session nobody created, else `{slot, key}` where `key` is the creator's LIVE session key when it is running, the edge the Sessions table nests on, and `null` otherwise; folded by `crew_log/session_tree.py`, see `crew-log-projection.md` section 6); `totals` carries `rss_mb`, `host_mb`, `host_pct`, `runtimes`, `rss_is_upper_bound`, `lineage_over_cap` (whether the store held more session logs than the capped lineage scan admits on this sample; the live sessions' logs are read first, so what went unread is closed sessions' logs; false within the cap or with the crew log off), `lineage_cap` (that cap, so the page can say "N+"; 0 with the crew log off); `history` is a ring of `{t, mb}` snapshots; `tasks` is the subagent task list from `SubagentManager.task_memory_rows()`)
...
**File diffs**: GET `/api/file-diff?path=...` returns the working-tree diff and HEAD content for the requested file, plus a `status` field — `not_git` (the repository preflight failed, so the file is outside any git work tree and there is no baseline; only the preflight may claim this), `untracked` (inside a repo, all-added `--no-index` diff — including every file of a freshly-initialized repo with no commits yet), `modified`, `clean`, or `error` (a git invocation past the preflight failed or timed out; distinct from `clean` so a git failure is never reported as "no changes"). Every reachable `status` value is served with HTTP 200; a sensitive path is 403 `{"error": "Access denied"}`, and the empty-path / file-not-found early returns reply `{"diff": "", "original": ""}` with no `status` key. Snapshot reads and every Git subprocess decode text explicitly as UTF-8 with replacement for malformed bytes, so Windows locale code pages cannot corrupt non-ASCII file content. Both text fields are credential-redacted through `redact_via_context`, the same pass `/api/file-read` applies to the panel's buffer, because the file panel diffs those two strings against each other and one raw side renders an untouched credential line as a hunk. The pass sees whole field text and neither field is truncated before it, because slicing first can cut the tail a credential pattern needs and serve the surviving prefix as real bytes; that is also why the pass runs off the event loop. `status` is computed from the unredacted `git diff`, so it still reports `modified` when the only change is inside a redacted span.
