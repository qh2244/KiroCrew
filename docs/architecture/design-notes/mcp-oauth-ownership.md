...
So in practice we live on Path A. The cost: **kiro-cli owns the OAuth protocol
and token lifecycle** — config reading, browser flow, callback server, token
storage, and refresh. Kiro Crew receives one-directional `_kiro.dev/*`
notifications and can stat or unlink exact artifacts for known URLs, but never
reads their contents. Concretely:

- We can answer grant presence only for a **known MCP URL**; we cannot turn the opaque cache into a list of authenticated servers or account identities.
- We can't proactively refresh tokens or check expiry.
- We can remove one known, exclusively owned server's local artifact pair, but cannot revoke its token at the provider or inspect which account it represented.
...
kiro-cli owns the OAuth chain end to end and Kiro Crew never reads or holds the
credential. Kiro Crew can observe that a known grant exists by `stat` on the paired
token and registration artifacts, and can locally remove an ownership-proven,
unshared pair by `unlink`; it cannot inspect, refresh, or revoke the credential at
the provider. A design that would move custody into Kiro Crew is a proposal, not
current behaviour, and is argued in
