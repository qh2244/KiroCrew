...
import { renderWithProviders, createTestStore, composerRoot, composerSelection, composerValue, setComposerValue, pressInComposer, awaitComposer } from './helpers'
...
describe('Select-to-Ask', () => {
  ...
  it('SideChat renders a seed written into its slot as a grounding quote, and puts the caret after it', async () => {
    ...
    await waitFor(() => expect(document.querySelector('[data-side-chat-input] [data-composer-input]')).not.toBeNull())
    await act(async () => {})
    const root = composerRoot(document.querySelector('[data-side-chat-input]') as HTMLElement)
    expect(composerValue(root)).toBe('')
    act(() => { seedSideChatDraft(SLOT, 'line one\nline two') })
    await waitFor(() => expect(composerValue(root)).toBe('> line one\n> line two\n\n'))
    // The nudge (Opus finding on 51d533776): it probed `textarea[...]`, found
    // nothing on the Lexical root, skipped focus + caret and still consumed the
    // seed. Now it places the caret after the quote through the root's control
    // handle and focuses the editable root.
    await waitFor(() => expect(document.activeElement).toBe(root))
    const len = '> line one\n> line two\n\n'.length
    expect(composerSelection(root)).toEqual({ start: len, end: len })
  })

  it('a seed for another slot never shows in this panel; a seed written BEFORE mount is waiting when it mounts', async () => {
    const SLOT = 'seed-slot'
    const store = createTestStore({
      dashboard: dashInitial,
      chat: { ...chatInitial, activeSlot: SLOT, slotHistory: [SLOT], activityOpen: true, activityTab: 'side' } as unknown as RootState['chat'],
    ...
    renderWithProviders(<SideChat slot={SLOT} />, { store })
    const ta = await awaitComposer(document.querySelector('[data-side-chat-input]') as HTMLElement)
    expect(composerValue(ta)).toBe('> asked before mount\n\n')
  ...
  it('a Side Chat draft survives the panel unmounting and is kept per slot', async () => {
    ...
    const side = () => document.querySelector('[data-side-chat-input]') as HTMLElement
    const ta = () => composerRoot(side())
    await awaitComposer(side())
    await setComposerValue('half a question', ta())
    expect(composerValue(ta())).toBe('half a question')
    first.unmount()
    ...
    await awaitComposer(side())
    await waitFor(() => expect(composerValue(ta())).toBe('half a question'))
  })
  ...
  it('a failed submit hands the text back to the slot it was SUBMITTED for, not the slot now shown', async () => {
    ...
    const ta = () => composerRoot(side())
    await awaitComposer(side())
    await setComposerValue('question for A', ta())
    pressInComposer('Enter', { code: 'Enter' }, ta())
    ...
    await act(async () => {})
    await setComposerValue('typing in B', ta())
    ...
    expect(composerValue(ta())).toBe('typing in B')
    ...
    await act(async () => {})
    await waitFor(() => expect(composerValue(ta())).toContain('question for A'))
    await setComposerValue(`${composerValue(ta())} and more`, ta())
