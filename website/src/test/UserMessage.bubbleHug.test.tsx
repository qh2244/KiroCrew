...
describe('sent bubble sizing', () => {
  ...
  it('the steer wrapper shrink-wraps too, so both bubbles share one end edge', () => {
    // The wrapper must carry the SAME cap as the bubble: during intrinsic
    // sizing a percentage max-width is treated as none, so a wrapper whose cap
    // differed from the bubble's inflated to the full column and the capped
    // bubble inside landed at its left edge. Both now carry the column cap
    // (`max-w-full`, #8398), so they resolve to one width and one end edge, and
    // 100% still caps a phone viewport.
