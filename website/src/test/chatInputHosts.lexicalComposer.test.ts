...
describe('ChatInput hosts — lexicalComposer is decided at every product mount', () => {
  ...
  // `lexicalComposer={!touchDevice}` fed by `useIsTouchDevice()` — so lifting it
  // later is a deliberate edit here, not a silent drift on one host.
  it('every product <ChatInput> mount gates the rich composer on the touch-device signal', () => {
    const ungated = mountsByFile.flatMap(([f, mounts]) => {
      const src = readFileSync(f, 'utf-8')
      const hookCalled = /\buseIsTouchDevice\(\)/.test(src)
      return mounts
