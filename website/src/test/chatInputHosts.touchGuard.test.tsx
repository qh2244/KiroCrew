...
// `useTouchDeviceAtMount` reads this predicate once at mount, so mocking the
...
describe('chat hosts — rich composer is gated on the touch-device signal', () => {
  ...
  it('the decision is latched at mount: a pointer-capability change mid-session does not swap the editor', async () => {
    // A mouse attached to a tablet (or a keyboard detached from a convertible)
    // flips `(pointer: coarse)` / `(hover: none)` while a draft may be live. The
    // reactive `useIsTouchDevice()` would re-read the predicate on the next render
    // and hard-swap `<LexicalComposerInput>` for the `<textarea>` — caret,
    // selection, IME composition and undo history gone. The hosts read the
    // capability once, at mount.
    touchEnv.touch = false
    const { rerender } = renderWithProviders(<SideChat slot={SLOT} />, { store: store() })
    const before = await composerElement()
    expect(before.getAttribute('contenteditable')).toBe('true')

    touchEnv.touch = true
    rerender(<SideChat slot={SLOT} />)
    const after = await composerElement()
    expect(after.getAttribute('contenteditable')).toBe('true')
    expect(after.tagName).not.toBe('TEXTAREA')
  })
})
