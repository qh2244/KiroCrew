...
describe("crew webview containment", () => {
  ...
  it("tells a failed refresh apart from a failed render, and never covers the document", async () => {
    ...
    // One record, one age: this re-mint failed for the SAME record (new html,
    // same `published_at`), so the bar's chip keeps the plain label. UX review's
    // reader saw "New version published 3d ago" over "Showing the version from
    // 3d ago" in exactly this state and could not tell which copy was on screen
    // -- the "new version" label belongs to the republished case pinned in the
    // next test, not to every failed refresh. Resolved through the collapse
    // button's row so the docked card's chip (same test id) is not the one read.
    // Stems from the catalog again, so a copy edit moves the pin.
    const collapseBtn = document.querySelector(
      '[data-testid="crew-webview-collapse"]',
    ) as Element;
    const barChip = collapseBtn.parentElement?.querySelector(
      '[data-testid="crew-webview-age"]',
    ) as Element;
    expect(barChip).not.toBeNull();
    const newerStem = copy.webview_published_ago_newer.split("{{")[0];
    const plainStem = copy.webview_published_ago.split("{{")[0];
    expect(newerStem.length).toBeGreaterThan(5);
    expect(newerStem).not.toBe(plainStem);
    expect((barChip.textContent || "").startsWith(plainStem)).toBe(true);
    expect((barChip.textContent || "").startsWith(newerStem)).toBe(false);
  });
