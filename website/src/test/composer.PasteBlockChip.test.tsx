...
describe('pasteSnippet', () => {
  ...
  it('does not split the whole payload to derive either label', () => {
    const huge = `early label\n${'tail\n'.repeat(100_000)}`
    const split = vi.spyOn(String.prototype, 'split')
    try {
      expect(pasteFirstLine(huge)).toBe('early label')
      expect(pasteSnippet(huge)).toBe('early label')
      const splitReceivers = split.mock.instances.map((receiver) => String(receiver))
      expect(splitReceivers).not.toContain(huge)
    } finally {
      split.mockRestore()
    }
  })

  it('stops after a bounded prefix of leading blank lines', () => {
    const firstNonBlankBeyondPrefix = `${' \n'.repeat(3_000)}too late`
    expect(pasteFirstLine(firstNonBlankBeyondPrefix)).toBe('')
    expect(pasteSnippet(firstNonBlankBeyondPrefix)).toBe('')
  })
})
...
describe('PasteBlockChip snippet label', () => {
  ...
  it('names the three hidden actions: hover title and accessible description say "Click to edit · drag to reorder · Backspace removes"', () => {
    ...
    expect(chip.getAttribute('title')).toBe('def main():\nClick to edit · drag to reorder · Backspace removes')
    expect(chip).toHaveAccessibleDescription('Click to edit · drag to reorder · Backspace removes')
  })
