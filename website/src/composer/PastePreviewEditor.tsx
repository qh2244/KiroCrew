...
export default function PastePreviewEditor({
  ...
  useEffect(() => { if (open) { userSizedRef.current = false; setPanelWidth(PANEL_WIDTH) } }, [open])
  ...
  useLayoutEffect(() => {
    ...
    const compute = () => {
      ...
      setPos((prev) => (prev && prev.left === left && prev.above === above && prev.top === (above ? -1 : anchorRect.bottom + GAP))
        ? prev
  ...
  const onResizePointerDown = (e: React.PointerEvent<HTMLDivElement>) => {
    ...
    const maxH = maxTextareaRef.current
    ...
    const onMove = (ev: PointerEvent) => {
      ...
      setPanelWidth(Math.max(MIN_PANEL_WIDTH, Math.min(maxW, startW + dx)))
      setTextareaHeight(Math.max(MIN_TEXTAREA_PX, Math.min(maxH, startH + dy)))
    }
    const onUp = () => {
    ...
    open && anchorRect ? (
        ...
        >
          ...
          <textarea
            ...
            aria-label={i18nT('components.pastePreviewEditor.editor_aria')}
            className="font-mono text-[11px] leading-[1.5] text-text bg-bg border border-border rounded-sm p-2 outline-none focus:border-accent"
            style={{
              whiteSpace: 'pre',
              overflow: 'auto',
              resize: 'none',
          ...
          <div
            ...
            className={`absolute right-0 h-4 w-4 touch-none text-muted ${pos?.above ? 'top-0 cursor-nesw-resize' : 'bottom-0 cursor-nwse-resize'}`}
            style={{
