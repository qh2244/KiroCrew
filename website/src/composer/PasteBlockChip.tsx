...
/**
 * Maximum input examined for a first-line label. A first non-blank line after
 * 4 KiB of leading whitespace is not useful enough to justify scanning an
 * arbitrarily large paste on the UI thread.
 */
const FIRST_LINE_SCAN_CHARS = 4096

/**
 * The paste's first non-blank line with inner whitespace collapsed. The scan
 * is bounded to `FIRST_LINE_SCAN_CHARS`; a longer first line returns the part
 * that fits, and a non-blank line beyond the prefix is ignored. Feeds the
 * hover title, where the line is not subject to the visible label's 80-char
 * cap. Empty when the scanned prefix has no non-blank line.
 */
export function pasteFirstLine(content: string): string {
  const prefix = content.slice(0, FIRST_LINE_SCAN_CHARS)
  let start = 0

  while (start < prefix.length) {
    const newline = prefix.indexOf('\n', start)
    const end = newline === -1 ? prefix.length : newline
    const line = prefix.slice(start, end).replace(/\s+/g, ' ').trim()
    if (line) return line
    if (newline === -1) break
    start = newline + 1
  }
 ...
 * so a 10k-char first line never lands in the DOM as text. Empty when the
 * bounded scan finds no non-blank line.
 */
...
export interface PasteBlockChipProps {
  ...
  snippet?: string
  /** The normalized first line from the bounded label scan (see
   *  `pasteFirstLine`), for the hover title; defaults to `snippet`. */
  firstLine?: string
 ...
 * - The hover title carries the uncapped result of the bounded first-line scan
 *   (the visible label is cut at 80 chars) and then names the three actions the
 *   pill hides — "Click to edit · drag to reorder · Backspace removes" — because a label that merely restates
 *   itself leaves the feature discoverable only by trying, and the ✕ is out of
 *   the tab order, so a keyboard or AT user learns removal only from this hint.
 *   The same hint is the pill's `aria-describedby` text.
 *
...
function PasteBlockChip({
  ...
  return (
    <span
      ...
      className={`inline-flex items-center gap-1.5 px-2 py-1 rounded border border-border bg-bg-hover text-[12px] text-text cursor-pointer select-none align-baseline transition-colors focus:outline-hidden focus-visible:ring-1 focus-visible:ring-accent ${stateClass}${className ? ` ${className}` : ''}`}
    >
