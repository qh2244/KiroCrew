...
// ChatInput (src/composer/__harness__/chatinput.html) in a real browser —
// Chromium by default; pass `firefox` or `webkit` as the third argument (or
// PILLS_BROWSER=<engine>) to walk the same checks on another engine.
//
//   cd website && npx vite --port 5199 --strictPort --host 127.0.0.1 &
//   node scripts/capture-composer-pills.mjs [outDir] [baseUrl] [engine]
//
// Every step asserts; the script exits non-zero if any check fails. A step whose
// driver exists only on one engine (the CDP-driven IME composition) is recorded
// as SKIPPED on the others, never as passed.
...
const ENGINE = process.argv[4] || process.env.PILLS_BROWSER || 'chromium';
const ENGINES = { chromium, firefox, webkit };
if (!ENGINES[ENGINE]) { console.error(`unknown engine "${ENGINE}" — expected chromium, firefox or webkit`); process.exit(2); }
const VIDEO_DIR = `${OUT}/video`;
mkdirSync(OUT, { recursive: true });
const W = 1000, H = 720;
// Firefox: the async clipboard testing prefs let `navigator.clipboard.writeText`
// run without a user gesture, which the real-clipboard paste path below relies on.
const FIREFOX_PREFS = { 'dom.events.testing.asyncClipboard': true, 'dom.events.asyncClipboard.readText': true, 'dom.events.asyncClipboard.clipboardItem': true };
const browser = await ENGINES[ENGINE].launch({ headless: true, ...(ENGINE === 'firefox' ? { firefoxUserPrefs: FIREFOX_PREFS } : {}) });
// Clipboard permission grants are a Chromium-only Playwright API; the copy check
// below reads the `copy` event's own DataTransfer, so no engine needs them to pass.
const ctx = await browser.newContext({ viewport: { width: W, height: H }, ...(ENGINE === 'chromium' ? { permissions: ['clipboard-read', 'clipboard-write'] } : {}), recordVideo: { dir: VIDEO_DIR, size: { width: W, height: H } } });
...
const R = { engine: ENGINE, browserVersion: browser.version(), steps: [] };
...
const skip = (id, title, why) => { R.steps.push({ id, title, ok: null, skipped: true, detail: why }); console.log('SKIP ' + id + ' ' + title + ' — ' + why); };
...
// Chromium delivers a script-built DataTransfer on a synthetic paste event. Firefox
// empties a script-built DataTransfer the moment it is attached to an event (the
// listener sees `types: []`), so there the text goes through the real clipboard
// (`navigator.clipboard.writeText`, with the testing prefs above lifting the
// user-activation requirement) and a real Ctrl+V into the focused composer.
const paste = ENGINE === 'chromium'
  ? (t) => page.evaluate((t) => { const dt = new DataTransfer(); dt.setData('text/plain', t); document.querySelector('[data-composer-input]').dispatchEvent(new ClipboardEvent('paste', { bubbles: true, cancelable: true, clipboardData: dt })); }, t)
  : async (t) => { await page.evaluate((t) => navigator.clipboard.writeText(t), t); await page.keyboard.press('Control+V'); };
...
// The pointer path stays inside the viewport: a real pointer cannot leave the
// window either, and Firefox delivers a captured pointermove whose coordinates
// fall outside the viewport with a different position than Chromium does. The
// width clamp is still exercised — the handle sits close enough to the right
// edge that the grow drag reaches `maxW` before the pointer reaches the edge.
const drag = async (dx, dy) => { const h = await handle.boundingBox(); const x = h.x + h.width / 2, y = h.y + h.height / 2; await page.mouse.move(x, y); await page.mouse.down(); const steps = 12; for (let i = 1; i <= steps; i++) { await page.mouse.move(Math.min(W - 2, Math.max(2, x + dx * i / steps)), Math.min(H - 2, Math.max(2, y + dy * i / steps))); await page.waitForTimeout(25); } await page.mouse.up(); await pause(300); };
...
if (ENGINE === 'chromium') {
  ...
  // Playwright drives IME composition through CDP (Input.imeSetComposition); no
  // other engine exposes a composition driver, so this check is Chromium-only.
  skip('17', 'Chinese IME: during composition the value is not committed; commit inserts 你好 after the pill, pill intact', `no IME composition driver on ${ENGINE} (CDP only)`);
  // Steps 18/19 start from the committed state "[ Paste ] 你好"; insert the same
  // text through the keyboard so the engines compare like with like.
  await page.keyboard.type('你好'); await pause(400);
}
...
step('22', 'Live line count follows the edit; a dirty Escape keeps the panel open with the unsaved hint; a second Escape discards and the block is unchanged', liveCount === '5 lines' && hint === 'Unsaved changes — press Esc again to discard, click away to keep' && openAfterFirstEsc === 1 && (await page.locator('[role=dialog]').count()) === 0 && b22.lines === 4, { liveCount, hint, openAfterFirstEsc, lines: b22.lines });
...
// ---------- 28. The snippet-less fallback pill ----------
// A paste can never produce it: the paste handler strips trailing blank lines,
// so text with no printable character never collapses (checked first). The
// fallback label is for a HOST-provided block with no printable line — a
// restored draft, an external writer — so the harness seeds one.
await caption(31, 'A paste with no printable character does not collapse. A restored block with no printable line still gets a pill, labelled "Pasted text · N lines".');
await typeSlow('Trailing whitespace paste: '); await paste('   \n\n \n\n'); await pause(400);
const s28a = { pills: await page.locator('[data-paste-seq]').count(), blocks: (await blocks()).length };
await page.keyboard.press('Control+A'); await page.keyboard.press('Backspace'); await pause(200);
await page.evaluate(() => { window.__ci.setBlocks([{ id: 'ws-only', seq: 1, lines: 4, content: '   \n\n \n' }]); window.__ci.setValue('Restored draft: [ Paste #1 · 4 lines ] and its note'); });
await pause(600);
const s28 = await page.evaluate(() => { const p = document.querySelector('[data-paste-seq]'); if (!p) return null; return { label: p.textContent.trim(), name: p.getAttribute('aria-label'), snippet: !!p.querySelector('[data-testid="paste-chip-snippet"]') }; });
await shotComposer('34-snippetless-fallback-pill');
step('28', 'A whitespace-only paste never collapses; a host block with no printable line renders the plain "Pasted text · N lines" pill with no snippet element', s28a.pills === 0 && s28a.blocks === 0 && s28 && !s28.snippet && /^Pasted text · 4 lines/.test(s28.label) && s28.name === 'Pasted text · 4 lines', { s28a, s28 });
await page.evaluate(() => { window.__ci.setBlocks([]); window.__ci.setValue(''); }); await pause(200);

// ---------- 29. Dark theme ----------
// The harness boots light (the app's theme init sets data-theme="kiro-light"),
// so every frame above is light. Flip the same attributes the app uses and
// re-render a pill with its preview open, reading computed colours, not classes.
await caption(32, 'Dark theme: the pill and the open preview follow the theme tokens (computed colours, not class names).');
const themed = () => page.evaluate(() => { const pill = document.querySelector('[data-paste-seq]'); const dlg = document.querySelector('[role=dialog]'); const ta = dlg && dlg.querySelector('textarea'); const c = (el, prop) => el ? getComputedStyle(el)[prop] : null; return { pillColor: c(pill, 'color'), pillBg: c(pill, 'backgroundColor'), dialogBg: c(dlg, 'backgroundColor'), textareaBg: c(ta, 'backgroundColor'), textareaColor: c(ta, 'color') }; });
await typeSlow('Theme check '); await paste('t1\nt2\nt3\nt4'); await pause(300);
await page.click('[data-paste-seq="1"]'); await pause(500);
const light29 = await themed();
await page.evaluate(() => { document.documentElement.setAttribute('data-theme', 'dark'); document.documentElement.setAttribute('data-mode', 'dark'); }); await pause(400);
const dark29 = await themed();
await shotDialog('35-dark-theme-pill-and-preview');
const lum = (rgb) => { const m = /(\d+),\s*(\d+),\s*(\d+)/.exec(rgb || ''); return m ? (Number(m[1]) + Number(m[2]) + Number(m[3])) / 3 : null; };
step('29', 'Dark theme: pill text turns light and the preview panel/textarea turn dark — every surface follows the theme tokens, nothing is hardcoded', light29 && dark29 && lum(dark29.pillColor) > 180 && lum(light29.pillColor) < 120 && lum(dark29.dialogBg) < 80 && lum(light29.dialogBg) > 200 && lum(dark29.textareaBg) < 80 && lum(dark29.textareaColor) > 180, { light29, dark29 });
await page.keyboard.press('Escape'); await pause(200);
await page.evaluate(() => { document.documentElement.setAttribute('data-theme', 'kiro-light'); document.documentElement.setAttribute('data-mode', 'light'); window.__ci.setBlocks([]); window.__ci.setValue(''); }); await pause(300);

// ---------- 30. CLI mode ----------
// cli-mode.css indents `[data-composer-input]` AND `[data-composer-placeholder]`
// by 24px under `[data-ui="cli"] [style*="--mc-content-width"]`, so the `>`
// prompt glyph `[data-testid="input-wrapper"]::before` paints left of the
// placeholder, of typed text and of a pill. The harness wraps the composer in
// the same scope anchor the chat pane sets; only `data-ui` is flipped here.
await caption(33, 'CLI mode: the > prompt sits left of the indented placeholder, typed text and pills.');
await page.evaluate(() => document.documentElement.setAttribute('data-ui', 'cli')); await pause(400);
const cliEmpty = await page.evaluate(() => { const input = document.querySelector('[data-composer-input]'); const ph = document.querySelector('[data-composer-placeholder]'); const wrap = document.querySelector('[data-testid="input-wrapper"]'); return { inputPad: input && getComputedStyle(input).paddingLeft, phPad: ph && getComputedStyle(ph).paddingLeft, glyph: wrap && getComputedStyle(wrap, '::before').content, caret: input && getComputedStyle(input).caretColor }; });
await shotComposer('36-cli-mode-placeholder');
await typeSlow('CLI check '); await paste('c1\nc2\nc3\nc4'); await pause(300);
const cliPill = await page.evaluate(() => { const input = document.querySelector('[data-composer-input]'); const pill = document.querySelector('[data-paste-seq]'); const wrap = document.querySelector('[data-testid="input-wrapper"]'); const wr = wrap.getBoundingClientRect(); const ir = input.getBoundingClientRect(); const pr = pill && pill.getBoundingClientRect(); return { textStart: Math.round(ir.left + parseFloat(getComputedStyle(input).paddingLeft) - wr.left), pillLeft: pr ? Math.round(pr.left - wr.left) : null, pills: document.querySelectorAll('[data-paste-seq]').length }; });
await shotComposer('37-cli-mode-pill');
step('30', 'CLI mode: composer and placeholder carry the 24px indent and the accent caret, the `>` glyph paints, and typed text and the pill start right of it', cliEmpty.inputPad === '24px' && cliEmpty.phPad === '24px' && cliEmpty.glyph === '">"' && cliEmpty.caret !== 'auto' && cliPill.pills === 1 && cliPill.textStart >= 24 && cliPill.pillLeft >= 24, { cliEmpty, cliPill });
await page.evaluate(() => { document.documentElement.removeAttribute('data-ui'); window.__ci.setBlocks([]); window.__ci.setValue(''); }); await pause(300);

// ---------- 31. Unsaved hint at the minimum panel width ----------
// The two-exit hint is longer than the old one; at MIN_PANEL_WIDTH (280px) it
// must wrap inside its own flex item, never into Cancel/Save.
await caption(34, 'At the 280px minimum width the unsaved-changes hint wraps; Cancel and Save stay clear of it.');
await typeSlow('Narrow check '); await paste('n1\nn2\nn3\nn4'); await pause(300);
await page.click('[data-paste-seq="1"]'); await pause(500);
await drag(-400, 0); await pause(200);
await page.click('[role=dialog] textarea'); await page.keyboard.press('Control+End'); await page.keyboard.type('\nn5'); await pause(200);
await page.keyboard.press('Escape'); await pause(300);
const s31 = await page.evaluate(() => { const dlg = document.querySelector('[role=dialog]'); const hint = dlg && dlg.querySelector('[data-testid$="-unsaved"]'); const save = dlg && [...dlg.querySelectorAll('button')].find(b => /save/i.test(b.textContent)); const cancel = dlg && [...dlg.querySelectorAll('button')].find(b => /cancel/i.test(b.textContent)); const r = (el) => el && el.getBoundingClientRect(); const h = r(hint), sv = r(save), cn = r(cancel), d = r(dlg); const overlaps = (a, b) => a && b && a.right > b.left && a.left < b.right && a.bottom > b.top && a.top < b.bottom; return { width: d && Math.round(d.width), hintLines: h && Math.round(h.height / parseFloat(getComputedStyle(hint).lineHeight)), hintText: hint && hint.textContent, overlapsSave: !!overlaps(h, sv), overlapsCancel: !!overlaps(h, cn), saveInside: sv && d && sv.right <= d.right + 0.5 }; });
await shotDialog('38-min-width-unsaved-hint');
step('31', 'At the 280px minimum panel width the unsaved hint wraps onto extra lines and never overlaps Cancel or Save, which stay inside the panel', s31.width === 280 && s31.hintLines >= 2 && !s31.overlapsSave && !s31.overlapsCancel && s31.saveInside, s31);
await page.keyboard.press('Escape'); await pause(200);
await page.evaluate(() => { window.__ci.setBlocks([]); window.__ci.setValue(''); }); await pause(200);

// ---------- 32. Narrow pane: the hint stays on one line ----------
// #13812 holds the textarea's sigil hint to one line with a faded tail in a
// narrow split-view pane. The rich composer's placeholder is an overlay <div>,
// so the same rule lands on its classes: at 420px the hint is one line rect,
// `white-space: nowrap`, a linear-gradient mask; the empty box keeps its
// one-line height. The scope of the rule (status sentences and a caller's own
// placeholder keep wrapping) is the unit contract in
// ChatInput.placeholderOneLine.lexical.test.tsx; this step measures the pixels.
await caption(35, 'A narrow pane: the hint stays on one line with a faded tail; the box stays one line tall.');
await page.setViewportSize({ width: 420, height: H }); await pause(500);
const readOverlay = () => page.evaluate(() => {
  const ph = document.querySelector('[data-composer-placeholder]'); const input = document.querySelector('[data-composer-input]');
  const cs = getComputedStyle(ph); const range = document.createRange(); range.selectNodeContents(ph);
  return { text: ph.textContent, lines: range.getClientRects().length, whiteSpace: cs.whiteSpace, mask: (cs.maskImage || cs.webkitMaskImage || '').slice(0, 15), overlayScroll: ph.scrollHeight, overlayBox: ph.clientHeight, inputBox: input.clientHeight, viewport: window.innerWidth };
});
const s32 = await readOverlay();
await shotComposer('39-narrow-hint-one-line');
step('32', 'In a 420px pane the hint placeholder stays on one line with a faded tail (nowrap + linear-gradient mask) and the empty composer keeps its one-line height', s32.viewport === 420 && s32.lines === 1 && s32.whiteSpace === 'nowrap' && s32.mask === 'linear-gradient' && s32.overlayScroll === s32.overlayBox && s32.inputBox === s32.overlayBox, s32);
await page.setViewportSize({ width: W, height: H }); await pause(400);

await caption(36, 'Done — every check passed.'); await pause(1200);

R.errs = errs; R.pass = R.steps.filter(s => s.ok === true).length; R.fail = R.steps.filter(s => s.ok === false).length; R.skipped = R.steps.filter(s => s.skipped).length; R.total = R.steps.length;
writeFileSync(`${OUT}/results.json`, JSON.stringify(R, null, 2));
const videoPath = await page.video().path();
await ctx.close(); await browser.close();
console.log(`\n[${ENGINE} ${R.browserVersion}] ${R.pass}/${R.total} passed, ${R.fail} failed, ${R.skipped} skipped; page errors: ${errs.length}`);
